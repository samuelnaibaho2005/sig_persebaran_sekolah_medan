...
// ================================================================
// POST /api/schools/init/create-view — membuat view (admin only)
// Endpoint ini untuk inisialisasi pertama kali
// ================================================================
router.post('/init/create-view', async (req, res) => {
  try {
    const sql = `
    ...
    // Test view
    const testResult = await pool.query('SELECT COUNT(*) as total FROM v_sekolah_lengkap');
    const total = testResult.rows[0].total;
    
    res.json({
      success: true,
      message: `View v_sekolah_lengkap berhasil dibuat dengan ${total} record sekolah!`
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
